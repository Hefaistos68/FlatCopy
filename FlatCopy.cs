...
{										
	public class FlatCopy
	{
		/// <summary>
		/// Regex pattern to extract a index/number in a filename like "testfile (1).txt"
		/// </summary>
		static readonly String s_pattern = ".*.\\((\\d*)\\)\\..*";
		
		/// <summary>
		/// The options for the regex operation to find the number
		/// </summary>
		static readonly RegexOptions s_options = RegexOptions.IgnoreCase|RegexOptions.Singleline;

		/// <summary>
		/// and finally the regex itself
		/// </summary>
		static readonly Regex regex = new Regex(s_pattern, s_options);
		...
		private static bool MoveOrCopy(string sourceFile, string destinationFolder, string fileName, bool bMove, bool bRemoveEmpty, bool bUnique)
		{
			...
			// if its the same folder, just skip it
			...
			// check for existence of a possible (0) indexed file, which would mean that we had other files of the same name already
			...
			if(File.Exists(Path.Combine(destinationFolder, possibleFilename)))
			{
				...
				// find the last one in the current folder, incrementing the index until we find a unused one
