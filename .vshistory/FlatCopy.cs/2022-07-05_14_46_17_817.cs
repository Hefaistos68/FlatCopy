	...
	internal class Program
	{
		...
		{																	
			...
			if(args.Length > 1)
			{
				...
				{
					archiveDirectory = Path.GetDirectoryName(args[1]) ?? throw new DirectoryNotFoundException(args[1]);
			...
			try
			{
				...
				foreach (string currentFile in allFiles)
				{
					...
					MoveOrCopy(currentFile, archiveDirectory, fileName, bMove, bRemoveEmpty, bOnlyUnique);
				}
			}
			catch (Exception e)
			{
		...
		private static void MoveOrCopy(string sourceFile, string destinationFolder, string fileName, bool bMove, bool bRemoveEmpty, bool bUnique)
		{
			...
			while (File.Exists(destinationFile))
			{
				...
				// #Todo: check file size/contents for equality before discarding
				if (bUnique)
					return;
